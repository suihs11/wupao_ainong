...
public final class Utilities {
    ...
    static Bitmap zoomBitmap(Bitmap paramBitmap) {
        int i = 3 * (paramBitmap.getWidth() / 2);
        int j = 3 * (paramBitmap.getHeight() / 2);
        int k = paramBitmap.getWidth();
        int m = paramBitmap.getHeight();
        Matrix localMatrix = new Matrix();
        localMatrix.postScale(i / k, j / m);
        return Bitmap.createBitmap(paramBitmap, 0, 0, k, m, localMatrix, true);
    }

    static Drawable zoomDrawable(Drawable paramDrawable) {
        return new BitmapDrawable(zoomBitmap(drawableToBitmap(paramDrawable)));
    }
